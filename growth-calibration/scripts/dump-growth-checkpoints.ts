...
interface CliArgs {
   ...
   *  format: "surplusPolicy=fruit_priority_limited,fruitPriorityRedistributionFraction=0.50,cellExpansionRelax=1.5".
  ...
  overrideMassFlow?: {
    ...
    phaseAwareDivisionFraction?: number;        // 0..1
    ...
    // Iter 9 (SSOT #116) — phaseAwareMassGrowth.cellDivisionStepDemandFraction
  ...
  /** Iter 8 (SSOT #108) — canopy / sink balance override (cultivar-level).
   *  format: "slaScale=1.2,leafSinkScale=0.7".
...
function applyOverrideMassFlow(args: CliArgs): void {
  ...
  console.log(`[override] massFlow: surplusPolicy=${ov.surplusPolicy ?? '-'}, fruitPriorityRedistributionFraction=${ov.fruitPriorityRedistributionFraction ?? '-'}, capRelaxByPhase cellDiv/Exp/Ripen=${ov.cellDivisionRelax ?? '-'}/${ov.cellExpansionRelax ?? '-'}/${ov.ripeningRelax ?? '-'}, phaseAware enabled=${ov.phaseAwareEnabled ?? '-'} divFrac=${ov.phaseAwareDivisionFraction ?? '-'} expMul=${ov.phaseAwareExpansionMultiplier ?? '-'} ripMul=${ov.phaseAwareRipeningMultiplier ?? '-'} clockMode=${ov.phaseAwareClockMode ?? '-'} cellDivStepDemandFrac=${ov.phaseAwareCellDivStepDemandFraction ?? '-'}`);
}

/** Iter 8 (SSOT #108) — source override.
 *  Multiplies ACTIVE_MODEL.photosynthesis.LUE_gDM_per_mol_PAR (global, photosynthesis layer).
 *  Photosynthesis.ts default-reads ACTIVE_MODEL each call so mutation propagates. */
...
function applyOverrideCanopy(args: CliArgs): void {
  ...
  console.log(`[override] canopy: slaScale=${ov.slaScale ?? '-'} leafSinkScale=${ov.leafSinkScale ?? '-'} → cultivar=${args.cultivar} SLA=${target?.SLA.toFixed(4) ?? '-'} sinkStrengthLeaf=${target?.sinkStrengthLeaf.toFixed(4) ?? '-'}`);
}
