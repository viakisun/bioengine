...
/**
 * ★ Iter 31 Phase 10 — leaf의 _두 입력 vector_ 산출.
 *
 * stemFrame.normal (parallel-transport, phyllotaxy 표현) + droopDeg →
 * petiole world direction + blade up world.
 *
 * @param stemFrameNormal  Skeleton parallel-transport normal (radial outward)
 * @param droopDeg         blade plane droop (positive = 아래로, gravity)
 */
export function computeLeafPetioleAndBladeAxes(
  stemFrameNormal: { x: number; y: number; z: number },
  droopDeg: number,
): { petiole: { x: number; y: number; z: number }; bladeUp: { x: number; y: number; z: number } } {
  // Step 1: horizontal stem outward (project stemFrame.normal to xz-plane, normalize)
  const outwardHLen = Math.sqrt(stemFrameNormal.x ** 2 + stemFrameNormal.z ** 2);
  const outwardH = outwardHLen > 1e-6
    ? { x: stemFrameNormal.x / outwardHLen, y: 0, z: stemFrameNormal.z / outwardHLen }
    : { x: 1, y: 0, z: 0 };

  // Step 2: tilt axis (horizontal, perpendicular to outwardH)
  //   = cross(world-up, outwardH) — gravity drop direction
  const tiltAxis = cross3({ x: 0, y: 1, z: 0 }, outwardH);

  // Step 3: rotate outwardH down by droopDeg around tiltAxis → petiole direction
  // Step 4: rotate world-up down by same droopDeg around tiltAxis → blade up
  // (둘 다 _같은 rotation_ — leaf 전체가 가지 cantilever)
  //
  // Right-hand rule + tiltAxis = cross(up, outwardH):
  //   positive rad rotates outwardH _아래로_ (toward -Y). 정상 droop 방향.
  const rad = droopDeg * Math.PI / 180;  // positive droopDeg = 아래로 tilt
  const petiole = rotateAroundUnitAxis(outwardH, tiltAxis, rad);
  const bladeUp = rotateAroundUnitAxis({ x: 0, y: 1, z: 0 }, tiltAxis, rad);

  return { petiole, bladeUp };
}

/** Rodrigues rotation: v rotated around unit axis k by rad. */
function rotateAroundUnitAxis(v: { x: number; y: number; z: number }, k: { x: number; y: number; z: number }, rad: number): { x: number; y: number; z: number } {
  ...
  return {
    x: v.x * c + kxv.x * s + k.x * dot * (1 - c),
    y: v.y * c + kxv.y * s + k.y * dot * (1 - c),
    z: v.z * c + kxv.z * s + k.z * dot * (1 - c),
  };
}
