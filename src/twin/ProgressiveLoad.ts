...
export function createProgressiveLoad(deps: {
  ...
  async function runSequence(savedQuality: number, currentDay: number): Promise<void> {
    ...
    console.log(`[ProgressiveLoad] start (savedQuality=${savedQuality})`);
  ...
  return {
    start({ savedQuality, currentDay }) {
      ...
      if (running) {
        clearPending();
        console.log(`[ProgressiveLoad] aborted at stage=${currentStageName}  (restart)`);
      }
      running = true;
    ...
    abort(reason) {
      if (!running) return;
      clearPending();
      console.log(`[ProgressiveLoad] aborted at stage=${currentStageName}  (${reason})`);
      running = false;
    },
    isRunning: () => running,
  };
