  ...
  function buildFromState(state: PlantState, plantBase: PlantBase, cultivarKey: string) {
    ...
    (window as unknown as { __skinplantView?: (mode: 'stem' | 'truss' | 'leaf' | 'full') => void })
      .__skinplantView = (mode) => {
        ...
        log.debug(`mode=${mode} stem=${stemOn} leaf=${leafOn} truss=${truOn}`);
      };
    ...
    (window as unknown as { __dockingOverlay?: (opts: {
      ...
      log.debug(`enable=${opts.enable} edgeTypes=${merged.edgeTypes?.join(',')} focus=${merged.focus} labelMode=${merged.labelMode}`);
    };
    ...
    (window as unknown as { __leafWireframe?: (enable: boolean) => void })
      ...
      applyLeafWireframe();
      log.debug(`${enable ? 'ON' : 'OFF'} — ${currentParts.leaves.length} leaves`);
    };
    ...
    (window as unknown as { __skinplantPetioleDock?: (opts: {
      ...
      if (!opts.enable) {
        log.debug('OFF — JSON published only');
        return;
      }
    ...
    for (const edge of graph.edges.values()) {
      ...
      for (const anchor of edge.organAnchors) {
        ...
        // 적엽 (defoliation) 처리: leaf-only hide는 stem이 그대로 남아 부자연.
        //   대신 SinglePlantOverlay가 root.position.y를 낮춰 plant 전체를 _땅 속으로_
        //   내려보냄 → 시각상 하부 30cm 가려짐 (줄기 + 잎 모두).
  ...
  return {
    ...
    setLushEnabled(v) {
      ...
      if (import.meta.env?.DEV && !v) {
        ...
        if (orphans.length > 0) {
          log.warn(
            `${orphans.length} mesh(es) still enabled `
