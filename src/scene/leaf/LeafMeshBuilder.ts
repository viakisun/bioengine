...
// ─── Per-leaflet plane chunk (Iter 39 L9-A S79 — inlined from LeafletPlaneChunk.ts)
//
...
//
// Width source: ShapeProfileSample[] — buildShapeProfile이 생성한
// 좌우 halfWidth 시계열. lobe/serration noise는 호출자가 profile 자체에 add.
//
// 본 함수는 nonshape 부분만 유지: transverse cup + longitudinal droop +
// midrib + lateral vein + curl z-twist + waviness.
//
// ★ L9-A S79 — LeafletPlaneChunk.ts 별도 파일 삭제 + LeafMeshBuilder.ts 안으로
//   inline (원칙 #54 단일 organ SSOT). 외부 호출 0이었음. 산식 변경 0.

interface ShapeProfileSampleLite {
  u: number;
  halfWidthLeft: number;
  halfWidthRight: number;
}

interface LeafletPlaneOptions {
  ...
  isTerminal: boolean;
  /** Iter 39 Phase F2.5 — vein/midrib surface rendering.
   *  0 = 비활성 (legacy), 1 = botanical (midrib raise 강화 + lateral vein 음각). */
...
const LEAFLET_PLANE_COLS = LEAFLET_PLANE_WIDTH_SEGS * 2 + 1;  // 9 columns
...
export function buildLeafletPlaneChunk(
  ...
  for (let row = 0; row <= lengthSegs; row++) {
    ...
    for (let col = 0; col < COLS; col++) {
      ...
      // Iter 39 Phase F2.5 — midrib raise + lateral vein groove.
