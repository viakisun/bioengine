...
test.describe('Iter 36 v5 — Leaflet variation (4 position types)', () => {
  test('LEAF-ENGINE-MODULE-EXISTS-01: src/scene/leaf-engine/ 7 files 존재', async () => {
    const required = [
      'agePresets.ts',
      'correlationRules.ts',
      'shapeProfile.ts',
    ...
    for (const f of required) {
      const filePath = path.join(REPO_ROOT, 'src/scene/leaf-engine', f);
      const stat = await fs.stat(filePath).catch(() => null);
      expect(stat, `leaf-engine/${f} 존재 의무`).not.toBeNull();
    }
  });

  test('LEAF-ENGINE-5-PRESETS-01: AGE_PRESETS 5종 정의 (young/mature/old/complex/potato-leaf)', async () => {
    const src = await fs.readFile(
      path.join(REPO_ROOT, 'src/scene/leaf-engine/agePresets.ts'),
      'utf-8',
    );
  ...
  test('LEAF-ENGINE-CORRELATION-RULES-01: applyCorrelation 산식 (사용자 §8 묶음)', async () => {
    const src = await fs.readFile(
      path.join(REPO_ROOT, 'src/scene/leaf-engine/correlationRules.ts'),
      'utf-8',
    );
