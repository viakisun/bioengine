...
## 0. TL;DR — 보정 후 (P0+P1 6개 적용)

1. **초장 ✓** — W16 268.9cm (표준 270–300, 보정 전 208) — *gap -25% → 0%*
2. **줄기 두께 ✓** — W16 base 19.9mm 중앙값 17.3–22.7 분포 (표준 18–22, 보정 전 24mm 80% cap) — *양방향 갭 모두 해소*
3. **화방/수확 ✓** — W16 12.9 화방, 21.6 익은 과, 4.14 kg/식물 (모두 표준 부합)
4. **마디 수 살짝 과다** — 45.8 vs 34–42 — 절간 6.1cm 가 표준 8–10cm 보다 짧아 마디 더 빈번. 추가 보정 여지
5. **잎 수 여전히 과다** — 27.8 vs 16–22 (보정 전 34) — *gap +60% → +27%* 으로 큰 개선이지만 표준 초과 유지. 상위 노드 보존 정책 더 엄격하게 가능
6. **수확 시점 ✓** — 첫 화방 W6 (W4 0.2), 첫 익은 과 W11 (표준 W10–11) 정확

**현재 모델 신뢰도**: 핵심 정량 (수확량, 시점, 줄기 두께, 초장) 모두 표준 부합. 시각적 캐릭터 (잎 수, 마디 밀도) 는 살짝 빽빽한 인상이 남아 P2 에서 마무리.
